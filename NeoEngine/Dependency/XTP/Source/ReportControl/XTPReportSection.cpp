...
int CXTPReportSection::GetPageRowCount(CDC *pDC) const
{
	...
	for (int nRow=m_pRows->GetTopRowIndex(); nRow<m_pRows->GetCount(); nRow++)
	{
		CXTPReportRow *pRow = m_pRows->GetAt(nRow);
...
void CXTPReportSection::Draw(CDC *pDC)
{
	...
	int nIndex  = m_pRows->GetTopRowIndex();
	int nOffset = 0;

	if (m_pControl->m_scrollModeV  == xtpReportScrollModeSmooth &&
		 m_pControl->m_pSectionBody == this)
	{
		int nTop;

		if (GetRowAt(pDC, rcSection.Width(), m_pControl->m_nScrollOffsetV, nIndex, nTop))
		{
			nOffset = m_pControl->m_nScrollOffsetV - nTop;
		}
	}
...
BOOL CXTPReportSection::EnsureVisibleSmooth(CDC *pDC, CXTPReportRow *pRow)
{
	...
	// Scroll up
	if (nOffset < m_pControl->m_nScrollOffsetV)
	{
	...
	if (nOffset > m_pControl->m_nScrollOffsetV + GetRect().Height())
	{
...
int CXTPReportSection::EnsureVisibleCalc(CDC *pDC, CXTPReportRow *pRow)
{
	...
	int nIndexTop = GetRows()->GetTopRowIndex();
