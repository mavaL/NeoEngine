...
public:
	...
	{
		accu = "";
		Common::Signal<char(float, int, std::string)> sig1;
		...
		Foo foo;
		sig1 += Common::slot(foo, &Foo::foo_bool);
		sig1 += Common::slot(&foo, &Foo::foo_bool);
		sig1.emit(.5, 1, "12");

		Common::Signal<void(std::string, int)> sig2;
...
public:
	static void
		run()
	{
		Common::Signal<int(), Common::CollectorVector<int>> sig_vector;
...
public:
		...
		run()
	{
		...
		Common::Signal<bool(), Common::CollectorUntil0<bool>> sig_until0;
		sig_until0 += Common::slot(self, &TestCollectorUntil0::handler_true);
		sig_until0 += Common::slot(self, &TestCollectorUntil0::handler_false);
		sig_until0 += Common::slot(self, &TestCollectorUntil0::handler_abort);
...
public:
		...
		run()
	{
		...
		Common::Signal<bool(), Common::CollectorWhile0<bool>> sig_while0;
		sig_while0 += Common::slot(self, &TestCollectorWhile0::handler_0);
		sig_while0 += Common::slot(self, &TestCollectorWhile0::handler_1);
		sig_while0 += Common::slot(self, &TestCollectorWhile0::handler_abort);
		assert(!self.check1 && !self.check2);
...
bench_simple_signal()
{
	Common::Signal<void(void*, uint64_t)> sig_increment;
	...
	printf("OK\n  Benchmark: Common::Signal: %fns per emission (size=%zu): ", size_t(benchdone - benchstart) * 1.0 / size_t(i),
...
char *argv[])
{
	...
	printf("Signal/Benchmark: Common::Signal: ");
	bench_simple_signal();
	printf("OK\n");
