/********************************************************************
	...
	purpose:	Terrain system.

	history:	1. 2013.这是第一版,实现方式基本来自<<D3D11龙书>>第19章.
				2. 25/10/2016. From OGRE's terrain system.
*********************************************************************/
#ifndef Terrain_h__
#define Terrain_h__
...
namespace Neo
{
	...
	*/
	struct TerrainLayerSampler
	{
		...
		TerrainLayerSampler()
			: alias(""), format(ePF_Unknown)
		{
		}

		TerrainLayerSampler(const STRING& aliasName, ePixelFormat fmt)
	...
	*/
		...
		*/
		struct ImportData
		{
			...
			*/
			...
			new (for Image) and OGRE_ALLOC_T (for inputFloat), the latter
	...
	//			layerDeclaration = rhs.layerDeclaration;
				...
				{
					//if (rhs.inputImage)
					//	inputImage = new Image(*rhs.inputImage);
					//else
						inputImage = 0;

					if (rhs.inputFloat)
					{
						inputFloat = new float[terrainSize*terrainSize];
		...
		*/
		...
		public:
			...
			*/
			virtual void allocateVertexBuffers(Terrain* forTerrain, size_t numVertices, VertexBuffer** destPos, VertexBuffer** destDelta) = 0;
			/** Free (or return to the pool) vertex buffers for terrain.
			*/
			virtual void freeVertexBuffers(VertexBuffer* posbuf, VertexBuffer* deltabuf) = 0;
			...
			*/
			virtual IndexBuffer* getSharedIndexBuffer(uint16 batchSize,
				uint16 vdatasize, size_t vertexIncrement, uint16 xoffset, uint16 yoffset, uint16 numSkirtRowsCols,
				uint16 skirtRowColSkip, uint32& nIndexCount) = 0;
		...
		public:
			...
			~DefaultGpuBufferAllocator();
			void allocateVertexBuffers(Terrain* forTerrain, size_t numVertices, VertexBuffer** destPos, VertexBuffer** destDelta);
			void freeVertexBuffers(VertexBuffer* posbuf, VertexBuffer* deltabuf);
			IndexBuffer* getSharedIndexBuffer(uint16 batchSize,
				uint16 vdatasize, size_t vertexIncrement, uint16 xoffset, uint16 yoffset, uint16 numSkirtRowsCols,
		...
		protected:
			typedef std::list<VertexBuffer*> VBufList;
			VBufList mFreePosBufList;
			VBufList mFreeDeltaBufList;
			typedef std::map<uint32, IndexBuffer*> IBufMap;
			IBufMap mSharedIBufMap;

			uint32 hashIndexBuffer(uint16 batchSize,
				uint16 vdatasize, size_t vertexIncrement, uint16 xoffset, uint16 yoffset, uint16 numSkirtRowsCols,
				uint16 skirtRowColSkip);
			VertexBuffer* getVertexBuffer(VBufList& list, size_t vertexSize, size_t numVertices);
		...
		struct cBufferTerr
		{
			MAT44	posIndexToObjectSpace;
			VEC4	uvMul_0;
			VEC2	lodMorph;
			float	baseUVScale;
		};

		cBufferTerr		m_cbTerrain;
		ConstantBuffer*	m_pConstantBuf;
		...
		*/
		static void _populateIndexBuffer(uint32* pIndexes, uint16 batchSize,
			uint16 vdatasize, size_t vertexIncrement, uint16 xoffset, uint16 yoffset, uint16 numSkirtRowsCols,
			uint16 skirtRowColSkip);
		...
		*/
		void convertPosition(Space inSpace, const VEC3& inPos, Space outSpace, VEC3& outPos) const;
		/** Convert a position from one space to another with respect to this terrain.
		@param inSpace The space that inPos is expressed as
		@param inPos The incoming position
		@param outSpace The space which outPos should be expressed as
		...
		*/
		VEC3 convertPosition(Space inSpace, const VEC3& inPos, Space outSpace) const;
		...
		*/
		void convertDirection(Space inSpace, const VEC3& inDir, Space outSpace, VEC3& outDir) const;
		/** Convert a direction from one space to another with respect to this terrain.
		@param inSpace The space that inDir is expressed as
		@param inDir The incoming direction
		@param outSpace The space which outDir should be expressed as
		...
		*/
		VEC3 convertDirection(Space inSpace, const VEC3& inDir, Space outSpace) const;
	...
	//	void load(StreamSerialiser& stream);
		...
		*/
		float getHeightAtTerrainPosition(float x, float y) const;
		...
		of the terrain
		*/
		float getHeightAtWorldPosition(float x, float y, float z) const;
		...
		of the terrain
		*/
		float getHeightAtWorldPosition(const VEC3& pos) const;
		...
		*/
		...
		/** Get a VEC3 of the world-space point on the terrain, aligned as per
		options.
		@note This point is relative to Terrain::getPosition
		*/
		void getPoint(long x, long y, VEC3* outpos) const;
		...
		*/
		void getPointFromSelfOrNeighbour(long x, long y, VEC3* outpos) const;

		/** Get a VEC3 of the world-space point on the terrain, supplying the
		height data manually (can be more optimal).
		@note This point is relative to Terrain::getPosition
		*/
		void getPoint(long x, long y, float height, VEC3* outpos) const;
		...
		*/
		void getPointTransform(MAT44* outXform) const;
		...
		*/
		void getTerrainVector(const VEC3& inVec, VEC3* outVec) const;
		/** Translate a vector from world space to local terrain space based on a specified alignment.
		@param inVec The vector in basis space, where x/y represents the
		terrain plane and z represents the up vector
		*/
		void getTerrainVectorAlign(const VEC3& inVec, Alignment align, VEC3* outVec) const;

		/** Translate a vector from world space to local terrain space based on the alignment options.
		@param x, y, z The vector in basis space, where x/y represents the
		terrain plane and z represents the up vector
		*/
		void getTerrainVector(float x, float y, float z, VEC3* outVec) const;
		/** Translate a vector from world space to local terrain space based on a specified alignment.
		@param x, y, z The vector in world space, where x/y represents the
		terrain plane and z represents the up vector
		*/
		void getTerrainVectorAlign(float x, float y, float z, Alignment align, VEC3* outVec) const;

		/** Translate a vector into world space based on the alignment options.
		@param inVec The vector in basis space, where x/y represents the
		terrain plane and z represents the up vector
		*/
		void getVector(const VEC3& inVec, VEC3* outVec) const;
		/** Translate a vector into world space based on a specified alignment.
		@param inVec The vector in basis space, where x/y represents the
		terrain plane and z represents the up vector
		*/
		void getVectorAlign(const VEC3& inVec, Alignment align, VEC3* outVec) const;

		/** Translate a vector into world space based on the alignment options.
		@param x, y, z The vector in basis space, where x/y represents the
		terrain plane and z represents the up vector
		*/
		void getVector(float x, float y, float z, VEC3* outVec) const;
		/** Translate a vector into world space based on a specified alignment.
		@param x, y, z The vector in basis space, where x/y represents the
		terrain plane and z represents the up vector
		*/
		void getVectorAlign(float x, float y, float z, Alignment align, VEC3* outVec) const;
		...
		*/
		void getPosition(const VEC3& TSpos, VEC3* outWSpos) const;
		/** Convert a position from terrain basis space to world space.
		...
		@param outWSpos World space output position (setup according to current alignment).
		*/
		void getPosition(float x, float y, float z, VEC3* outWSpos) const;
		...
		height units.
		*/
		void getTerrainPosition(const VEC3& WSpos, VEC3* outTSpos) const;
		/** Convert a position from world space to terrain basis space.
		...
		height units.
		*/
		void getTerrainPosition(float x, float y, float z, VEC3* outTSpos) const;
		...
		*/
		void getPositionAlign(const VEC3& TSpos, Alignment align, VEC3* outWSpos) const;
		/** Convert a position from terrain basis space to world space based on a specified alignment.
		@param x,y,z Terrain space position, where (0,0) is the bottom-left of the
		terrain, and (1,1) is the top-right. The Z coordinate is in absolute
		height units.
		@param outWSpos World space output position (setup according to alignment).
		*/
		void getPositionAlign(float x, float y, float z, Alignment align, VEC3* outWSpos) const;
		...
		height units.
		*/
		void getTerrainPositionAlign(const VEC3& WSpos, Alignment align, VEC3* outTSpos) const;
		/** Convert a position from world space to terrain basis space based on a specified alignment.
		...
		height units.
		*/
		void getTerrainPositionAlign(float x, float y, float z, Alignment align, VEC3* outTSpos) const;
		...
		*/
		void setWorldSize(float newWorldSize);
		...
		*/
		void addLayer(float worldSize = 0, const StringVector* textureNames = 0);
		...
		layer. Leave blank to provide these later.
		*/
		void addLayer(uint8 index, float worldSize = 0, const StringVector* textureNames = 0);
		...
		*/
		...
		layer. Leave blank to provide these later.
		*/
		void replaceLayer(uint8 index, bool keepBlends, float worldSize = 0, const StringVector* textureNames = 0);
		...
		*/
		float getLayerWorldSize(uint8 index) const;
		/** How large an area in world space the texture in a terrain layer covers
		before repeating.
		@param index The layer index.
		@param size The world size of the texture before repeating
		*/
		void setLayerWorldSize(uint8 index, float size);
		...
		@param index The layer index.
		*/
		float getLayerUVMultiplier(uint8 index) const;
		...
		*/
		...
		const Texture* getLightmap() const { return mLightmap; }
		...
		*/
		...
		const Texture* getCompositeMap() const { return mCompositeMap; }
		...
		*/
	//	std::pair<bool, VEC3> rayIntersects(const Ray& ray, bool cascadeToNeighbours = false, float distanceLimit = 0); //const;
	...
	//	TerrainLayerBlendMap* getLayerBlendMap(uint8 layerIndex);
		...
		*/
		...
		const Texture* getGlobalColourMap() const { return mColourMap; }
		...
		*/
		void widenRectByVector(const VEC3& vec, const Rect& inRect, Rect& outRect);
		...
		@param outRect Output rectangle
		*/
		void widenRectByVector(const VEC3& vec, const Rect& inRect,
		...
		*/
		Texture* getLayerBlendTexture(uint8 index) const;
	...
	//	void handleGenerateMaterialResponse(const WorkQueue::Response* res, const WorkQueue* srcQ);
		...
		Texture* getTerrainNormalMap() const { return mTerrainNormalMap; }
		...
		*/
	//	Terrain* raySelectNeighbour(const Ray& ray, float distanceLimit = 0);
...
//		void _dumpTextures(const STRING& prefix, const STRING& suffix);
		...
		static void convertWorldToTerrainAxes(Alignment align, const VEC3& worldVec, VEC3* terrainVec);
		/// Utility method to convert axes from terrain space (xy terrain, z up) tp world space
		static void convertTerrainToWorldAxes(Alignment align, const VEC3& terrainVec, VEC3* worldVec);
	...
	protected:
		...
		*/
		uint32 getGeoDataSizeAtLod(uint16 lodLevel) const;
		...
		*/
		...
		void convertSpace(Space inSpace, const VEC3& inVec, Space outSpace, VEC3& outVec, bool translation) const;
		VEC3 convertWorldToTerrainAxes(const VEC3& inVec) const;
		VEC3 convertTerrainToWorldAxes(const VEC3& inVec) const;
		/** Get a VEC3 of the world-space point on the terrain, aligned Y-up always.
		@note This point is relative to Terrain::getPosition
		*/
		void getPointAlign(long x, long y, Alignment align, VEC3* outpos) const;
		/** Get a VEC3 of the world-space point on the terrain, supplying the
		height data manually (can be more optimal).
		@note This point is relative to Terrain::getPosition
		*/
		void getPointAlign(long x, long y, float height, Alignment align, VEC3* outpos) const;
		void calculateCurrentLod();
		/// Test a single quad of the terrain for ray intersection.
	//	std::pair<bool, VEC3> checkQuadIntersection(int x, int y, const Ray& ray); //const;
		...
		void clearGPUBlendChannel(uint8 index, uint32 channel);
	...
	*/
	...
	protected:
		...
		uint16 mLayerBlendMapSize;
		float mDefaultLayerTextureWorldSize;
		uint16 mDefaultGlobalColourMapSize;
		uint16 mLightmapSize;
		uint16 mCompositeMapSize;
		SColor mCompositeMapAmbient;
		SColor mCompositeMapDiffuse;
	...
	public:
		...
		*/
		void setSkirtSize(float skirtSz) { mSkirtSize = skirtSz; }
		/// Get the shadow map light direction to use (world space)
		const VEC3& getLightMapDirection() const { return mLightMapDir; }
		/** Set the shadow map light direction to use (world space). */
		void setLightMapDirection(const VEC3& v) { mLightMapDir = v; }
		/// Get the composite map ambient light to use 
		const SColor& getCompositeMapAmbient() const { return mCompositeMapAmbient; }
		/// Set the composite map ambient light to use 
		void setCompositeMapAmbient(const SColor& c) { mCompositeMapAmbient = c; }
		/// Get the composite map iffuse light to use 
		const SColor& getCompositeMapDiffuse() const { return mCompositeMapDiffuse; }
		/// Set the composite map diffuse light to use 
		void setCompositeMapDiffuse(const SColor& c) { mCompositeMapDiffuse = c; }
		/// Get the distance at which to start using a composite map if present
		float getCompositeMapDistance() const { return mCompositeMapDistance; }
		/// Set the distance at which to start using a composite map if present
		void setCompositeMapDistance(float c) { mCompositeMapDistance = c; }
		...
		*/
		...
		float getMaxPixelError() const { return mMaxPixelError; }
		...
		*/
		void setMaxPixelError(float pixerr) { mMaxPixelError = pixerr; }
		...
		*/
		float getDefaultLayerTextureWorldSize() const { return mDefaultLayerTextureWorldSize; }

		/** Set the default world size for a layer 'splat' texture to cover.
		*/
		void setDefaultLayerTextureWorldSize(float sz) { mDefaultLayerTextureWorldSize = sz; }
