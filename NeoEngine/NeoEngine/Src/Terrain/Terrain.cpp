...
namespace Neo
{
	...
	TerrainGlobalOptions::TerrainGlobalOptions()
		...
		, mCompositeMapAmbient(SColor::WHITE)
		, mCompositeMapDiffuse(SColor::WHITE)
		, mCompositeMapDistance(4000)
		, mUseVertexCompressionWhenAvailable(true)
	{
		mLightMapDir = VEC3(1, -1, 0);
		mLightMapDir.Normalize();
	}
	//---------------------------------------------------------------------
	const uint32 Terrain::TERRAIN_CHUNK_ID = makeIdentifier("TERR");
	const uint16 Terrain::TERRAIN_CHUNK_VERSION = 2;
	const uint32 Terrain::TERRAINGENERALINFO_CHUNK_ID = makeIdentifier("TGIN");
	const uint16 Terrain::TERRAINGENERALINFO_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERDECLARATION_CHUNK_ID = makeIdentifier("TDCL");
	const uint16 Terrain::TERRAINLAYERDECLARATION_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERSAMPLER_CHUNK_ID = makeIdentifier("TSAM");
	const uint16 Terrain::TERRAINLAYERSAMPLER_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERSAMPLERELEMENT_CHUNK_ID = makeIdentifier("TSEL");
	const uint16 Terrain::TERRAINLAYERSAMPLERELEMENT_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINLAYERINSTANCE_CHUNK_ID = makeIdentifier("TLIN");
	const uint16 Terrain::TERRAINLAYERINSTANCE_CHUNK_VERSION = 1;
	const uint32 Terrain::TERRAINDERIVEDDATA_CHUNK_ID = makeIdentifier("TDDA");
...

	{
		...
		m_pConstantBuf = g_env.pRenderer->GetRenderSys()->CreateConstantBuffer(sizeof(cBufferTerr), 10);
	}
	//---------------------------------------------------------------------
	Terrain::~Terrain()
	{
		...
		SAFE_DELETE(m_pConstantBuf);
	}
	//---------------------------------------------------------------------
	const AABB& Terrain::getAABB() const
	{
		return mQuadTree->getAABB();
	}
	//---------------------------------------------------------------------
	AABB Terrain::getWorldAABB() const
	{
	...
	//---------------------------------------------------------------------
	float Terrain::getBoundingRadius() const
	{
		if (!mQuadTree)
			return 0;
		else
...
//	}
	...
	bool Terrain::prepare(const ImportData& importData)
	{
		...
		mLodManager = new TerrainLodManager(this);

		copyGlobalOptions();

		// validate
		if (!(Common::IsPow2(importData.terrainSize - 1) && Common::IsPow2(importData.minBatchSize - 1)
			&& Common::IsPow2(importData.maxBatchSize - 1)))
		{
			_AST(0 && "terrainSize, minBatchSize and maxBatchSize must all be 2^n + 1 Terrain::prepare");
		}

		if (importData.minBatchSize > importData.maxBatchSize)
		{
			_AST(0 && "minBatchSize must be less than or equal to maxBatchSize Terrain::prepare");
		}
		...
		{
			if (Common::Equal(importData.inputBias, 0.0) && Common::Equal(importData.inputScale, 1.0))
			{
				// straight copy
		...
		mQuadTree = new TerrainQuadTreeNode(this, 0, 0, 0, mSize, mNumLodLevels - 1, 0, 0);
		mQuadTree->prepare();
	...
	{
		TerrainGlobalOptions& opts = *g_env.pSceneMgr->GetTerrainOptions();
	...
	void Terrain::determineLodLevels()
	{
		...
		*/
		mNumLodLevelsPerLeafNode = (uint16)(log2f(mMaxBatchSize - 1.0f) - log2f(mMinBatchSize - 1.0f) + 1.0f);
		mNumLodLevels = (uint16)(log2f(mSize - 1.0f) - log2f(mMinBatchSize - 1.0f) + 1.0f);
		...
		*/
		...
		while (depth-- && targetSplits)
		{
			...
			if (splits == targetSplits)
			{
				...
				uint32 sz = ((bakedresolution - 1) / splits) + 1;
	...
	void Terrain::load(int lodLevel, bool synchronous)
	{
		...
		_setMorphRequired(false);
		_setNormalMapRequired(true);
		_setLightMapRequired(false, false);
		_setCompositeMapRequired(false);
	...
	//---------------------------------------------------------------------
	float Terrain::getHeightAtTerrainPosition(float x, float y) const
	{
		...
		*/
		...
		VEC3 v0(startXTS, startYTS, getHeightAtPoint(startX, startY));
		VEC3 v1(endXTS, startYTS, getHeightAtPoint(endX, startY));
		VEC3 v2(endXTS, endYTS, getHeightAtPoint(endX, endY));
		VEC3 v3(startXTS, endYTS, getHeightAtPoint(startX, endY));
	...
	//---------------------------------------------------------------------
	float Terrain::getHeightAtWorldPosition(float x, float y, float z) const
	{
		VEC3 terrPos;
	...
	//---------------------------------------------------------------------
	float Terrain::getHeightAtWorldPosition(const VEC3& pos) const
	{
	...
	//---------------------------------------------------------------------
	VEC3 Terrain::convertPosition(Space inSpace, const VEC3& inPos, Space outSpace) const
	{
		VEC3 ret;
	...
	//---------------------------------------------------------------------
	VEC3 Terrain::convertDirection(Space inSpace, const VEC3& inDir, Space outSpace) const
	{
		VEC3 ret;
	...
	//---------------------------------------------------------------------
	void Terrain::convertPosition(Space inSpace, const VEC3& inPos, Space outSpace, VEC3& outPos) const
	{
		convertSpace(inSpace, inPos, outSpace, outPos, true);
	}
	//---------------------------------------------------------------------
	void Terrain::convertDirection(Space inSpace, const VEC3& inDir, Space outSpace, VEC3& outDir) const
	{
		convertSpace(inSpace, inDir, outSpace, outDir, false);
	}
	//---------------------------------------------------------------------
	void Terrain::convertSpace(Space inSpace, const VEC3& inVec, Space outSpace, VEC3& outVec, bool translation) const
	{
			...
			case TERRAIN_SPACE:
				...
				case POINT_SPACE:
					if (translation)
					{
						...
						outVec.x = static_cast<float>(static_cast<int>(outVec.x + 0.5));
						outVec.y = static_cast<float>(static_cast<int>(outVec.y + 0.5));
					}
	...
	//---------------------------------------------------------------------
	void Terrain::convertWorldToTerrainAxes(Alignment align, const VEC3& worldVec, VEC3* terrainVec)
	{
		switch (align)
		{
		case ALIGN_X_Z:
	...
	//---------------------------------------------------------------------
	void Terrain::convertTerrainToWorldAxes(Alignment align, const VEC3& terrainVec, VEC3* worldVec)
	{
		switch (align)
		{
		case ALIGN_X_Z:
	...
	//---------------------------------------------------------------------
	VEC3 Terrain::convertWorldToTerrainAxes(const VEC3& inVec) const
	{
		VEC3 ret;
	...
	//---------------------------------------------------------------------
	VEC3 Terrain::convertTerrainToWorldAxes(const VEC3& inVec) const
	{
		VEC3 ret;
	...
	//---------------------------------------------------------------------
	void Terrain::getPoint(long x, long y, VEC3* outpos) const
	{
		getPointAlign(x, y, mAlign, outpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPoint(long x, long y, float height, VEC3* outpos) const
	{
		getPointAlign(x, y, height, mAlign, outpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPointAlign(long x, long y, Alignment align, VEC3* outpos) const
	{
		getPointAlign(x, y, *getHeightData(x, y), align, outpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPointAlign(long x, long y, float height, Alignment align, VEC3* outpos) const
	{
		switch (align)
		{
		case ALIGN_X_Z:
	...
	//---------------------------------------------------------------------
	void Terrain::getPointTransform(MAT44* outXform) const
	{
		*outXform = MAT44::ZERO;
		switch (mAlign)
		{
	...
	//---------------------------------------------------------------------
	void Terrain::getVector(const VEC3& inVec, VEC3* outVec) const
	{
		getVectorAlign(inVec.x, inVec.y, inVec.z, mAlign, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getVector(float x, float y, float z, VEC3* outVec) const
	{
		getVectorAlign(x, y, z, mAlign, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getVectorAlign(const VEC3& inVec, Alignment align, VEC3* outVec) const
	{
		getVectorAlign(inVec.x, inVec.y, inVec.z, align, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getVectorAlign(float x, float y, float z, Alignment align, VEC3* outVec) const
	{
	...
	//---------------------------------------------------------------------
	void Terrain::getPosition(const VEC3& TSpos, VEC3* outWSpos) const
	{
		getPositionAlign(TSpos, mAlign, outWSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPosition(float x, float y, float z, VEC3* outWSpos) const
	{
		getPositionAlign(x, y, z, mAlign, outWSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainPosition(const VEC3& WSpos, VEC3* outTSpos) const
	{
		getTerrainPositionAlign(WSpos, mAlign, outTSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainPosition(float x, float y, float z, VEC3* outTSpos) const
	{
		getTerrainPositionAlign(x, y, z, mAlign, outTSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPositionAlign(const VEC3& TSpos, Alignment align, VEC3* outWSpos) const
	{
		getPositionAlign(TSpos.x, TSpos.y, TSpos.z, align, outWSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getPositionAlign(float x, float y, float z, Alignment align, VEC3* outWSpos) const
	{
		switch (align)
		{
		case ALIGN_X_Z:
	...
	//---------------------------------------------------------------------
	void Terrain::getTerrainPositionAlign(const VEC3& WSpos, Alignment align, VEC3* outTSpos) const
	{
		getTerrainPositionAlign(WSpos.x, WSpos.y, WSpos.z, align, outTSpos);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainPositionAlign(float x, float y, float z, Alignment align, VEC3* outTSpos) const
	{
		switch (align)
		{
		case ALIGN_X_Z:
	...
	//---------------------------------------------------------------------
	void Terrain::getTerrainVector(const VEC3& inVec, VEC3* outVec) const
	{
		getTerrainVectorAlign(inVec.x, inVec.y, inVec.z, mAlign, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainVector(float x, float y, float z, VEC3* outVec) const
	{
		getTerrainVectorAlign(x, y, z, mAlign, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainVectorAlign(const VEC3& inVec, Alignment align, VEC3* outVec) const
	{
		getTerrainVectorAlign(inVec.x, inVec.y, inVec.z, align, outVec);
	}
	//---------------------------------------------------------------------
	void Terrain::getTerrainVectorAlign(float x, float y, float z, Alignment align, VEC3* outVec) const
	{
	...
	//---------------------------------------------------------------------
	float Terrain::getWorldSize() const
	{
		return mWorldSize;
	}
	//---------------------------------------------------------------------
	float Terrain::getLayerWorldSize(uint8 index) const
	{
		...
		{
			return g_env.pSceneMgr->GetTerrainOptions()->getDefaultLayerTextureWorldSize();
		}
	}
	//---------------------------------------------------------------------
	void Terrain::setLayerWorldSize(uint8 index, float size)
	{
		if (index < mLayers.size())
		{
	...
	//---------------------------------------------------------------------
	float Terrain::getLayerUVMultiplier(uint8 index) const
	{
		if (index < mLayerUVMultiplier.size())
		{
	...
	const STRING& Terrain::getLayerTextureName(uint8 layerIndex, uint8 samplerIndex) const
	{
		...
		{
			_AST(0);
			return mLayers[0].textureNames[0];
		}
	...
	//---------------------------------------------------------------------
	void Terrain::setPosition(const VEC3& pos)
	{
		if (!pos.IsEquivalent(mPos))
		{
			mPos = pos;
			updateBaseScale();
			mModified = true;
		...
		bool synchronous, uint8 typeMask)
	{
		...
		ddres.remainingTypeMask = req.typeMask & DERIVED_DATA_ALL;
		...
		if (req.typeMask & DERIVED_DATA_DELTAS)
		{
			ddres.deltaUpdateRect = calculateHeightDeltas(req.dirtyRect);
			ddres.remainingTypeMask &= ~DERIVED_DATA_DELTAS;
		}
		else if (req.typeMask & DERIVED_DATA_NORMALS)
		{
			ddres.normalMapBox = calculateNormals(req.dirtyRect, ddres.normalUpdateRect);
			ddres.remainingTypeMask &= ~DERIVED_DATA_NORMALS;
		}
		else if (req.typeMask & DERIVED_DATA_LIGHTMAP)
		{
			ddres.lightMapBox = calculateLightmap(req.dirtyRect, req.lightmapExtraDirtyRect, ddres.lightmapUpdateRect);
		...
		if ((req.typeMask & DERIVED_DATA_NORMALS) &&
			!(ddres.remainingTypeMask & DERIVED_DATA_NORMALS))
		{
			finaliseNormals(ddres.normalUpdateRect, ddres.normalMapBox);
			mCompositeMapDirtyRect.Merge(req.dirtyRect);
		}
		if ((req.typeMask & DERIVED_DATA_LIGHTMAP) &&
			!(ddres.remainingTypeMask & DERIVED_DATA_LIGHTMAP))
		{
		...
		if (ddres.remainingTypeMask)
			newLightmapExtraRect.Merge(req.lightmapExtraDirtyRect);
		if (mDerivedUpdatePendingMask)
		{
	...
	Rect Terrain::calculateHeightDeltas(const Rect& rect)
	{
		...
		clampedRect.left = std::max(0, clampedRect.left);
		clampedRect.top = std::max(0, clampedRect.top);
		clampedRect.right = std::min((int)mSize, clampedRect.right);
		clampedRect.bottom = std::min((int)mSize, clampedRect.bottom);
	...
	//---------------------------------------------------------------------
	uint32 Terrain::getGeoDataSizeAtLod(uint16 lodLevel) const
	{
		uint32 size = getResolutionAtLod(lodLevel);
		uint32 prevSize = (lodLevel<mNumLodLevels - 1) ? getResolutionAtLod(lodLevel + 1) : 0;
		return size*size - prevSize*prevSize;
	}
	//---------------------------------------------------------------------
	void Terrain::calculateCurrentLod()
	{
		if (mQuadTree)
		{
	...
	//---------------------------------------------------------------------
	//std::pair<bool, VEC3> Terrain::rayIntersects(const Ray& ray,
	//	bool cascadeToNeighbours /* = false */, float distanceLimit /* = 0 */)
	//{
	//	typedef std::pair<bool, VEC3> Result;
	...
	//	VEC3 rayOrigin = ray.getOrigin() - getPosition();
	...
	//	AxisAlignedBox aabb(VEC3(0, minHeight, 0), VEC3(mSize, maxHeight, mSize));
	//	std::pair<bool, float> aabbTest = localRay.intersects(aabb);
	//	if (!aabbTest.first)
	//	{
	...
	//		}
	//		return Result(false, VEC3());
	//	}
	//	// get intersection point and move inside
	//	VEC3 cur = localRay.getPoint(aabbTest.second);
	...
	//		float xDist = Math::RealEqual(rayDirection.x, 0.0) ? dummyHighValue :
	//			(quadX - cur.x + flipX) / rayDirection.x;
	//		float zDist = Math::RealEqual(rayDirection.z, 0.0) ? dummyHighValue :
	...
	//}
	//---------------------------------------------------------------------
	//std::pair<bool, VEC3> Terrain::checkQuadIntersection(int x, int z, const Ray& ray)
	//{
	//	// build the two planes belonging to the quad's triangles
	//	VEC3 v1((float)x, *getHeightData(x, z), (float)z);
	//	VEC3 v2((float)x + 1, *getHeightData(x + 1, z), (float)z);
	//	VEC3 v3((float)x, *getHeightData(x, z + 1), (float)z + 1);
	//	VEC3 v4((float)x + 1, *getHeightData(x + 1, z + 1), (float)z + 1);
	...
	//	return std::pair<bool, VEC3>(false, VEC3());
	//}
	//---------------------------------------------------------------------
	...
	//---------------------------------------------------------------------
	void Terrain::addLayer(float worldSize, const StringVector* textureNames)
	{
	...
	void Terrain::shiftUpGPUBlendChannels(uint8 index)
	{
		...
		_AST(mBlendTextureList.size() == getBlendTextureCount(getLayerCount()));
	...
	//---------------------------------------------------------------------
	void Terrain::clearGPUBlendChannel(uint8 index, uint32 channel)
	{
		_AST(0);
	...
	void Terrain::createGPUBlendTextures()
	{
		...
		{
			mBlendTextureList.back()->Release();
		...
		for (uint8 i = currentTex; i < numTex; ++i)
		{
			...
			{
				// Load blend data
				PixelBox src(mLayerBlendMapSize, mLayerBlendMapSize, Texture::GetBytesPerPixelFromFormat(ePF_A8R8G8B8), mCpuBlendMapStorage[i]);
				mBlendTextureList[i]->UpdateRegion(nullptr, nullptr, &src);
				// release CPU copy, don't need it anymore
				delete []mCpuBlendMapStorage[i];
			}
			else
			{
				// initialise black
	...
	void Terrain::createOrDestroyGPUNormalMap()
	{
		...
		{
			// create
			const char* pData = mCpuTerrainNormalMap ? (char*)mCpuTerrainNormalMap->GetDataPointer() : nullptr;
			mTerrainNormalMap = g_env.pRenderer->GetRenderSys()->CreateTextureManual(mSize, mSize, pData, ePF_A8R8G8B8, 0, false);
	...
	{
		_AST(index < mBlendTextureList.size());
	...
	{
		_AST(layerIndex > 0 && layerIndex < mLayers.size());
		uint8 idx = layerIndex - 1;
	...
	PixelBox* Terrain::calculateNormals(const Rect &rect, Rect& finalRect)
	{
		...
		PixelBox* pixbox = new PixelBox(widenedRect.GetWidth(), widenedRect.GetHeight(), 4);
		uint8* pData = (uint8*)pixbox->GetDataPointer();
		...
		for (long y = widenedRect.top; y < widenedRect.bottom; ++y)
		{
			...
			{
				VEC3 cumulativeNormal = VEC3::ZERO;

				// Build points to sample
				VEC3 centrePoint;
				VEC3 adjacentPoints[8];
	...
	//---------------------------------------------------------------------
	void Terrain::finaliseNormals(const Rect &rect, PixelBox *normalsBox)
	{
		createOrDestroyGPUNormalMap();
		...
		if (mTerrainNormalMap)
		{
			...
			{
				mTerrainNormalMap->UpdateRegion(nullptr, nullptr, normalsBox);
			}
			else
			{
				...
				//mTerrainNormalMap->UpdateRegion(*normalsBox, dstBox);
				_AST(0);
			}
		}
		...
		// delete memory
		SAFE_DELETE(normalsBox);
	}
	//---------------------------------------------------------------------
	void Terrain::widenRectByVector(const VEC3& vec, const Rect& inRect, Rect& outRect)
	{
		widenRectByVector(vec, inRect, getMinHeight(), getMaxHeight(), outRect);
	}
	//---------------------------------------------------------------------
	void Terrain::widenRectByVector(const VEC3& vec, const Rect& inRect,
		float minHeight, float maxHeight, Rect& outRect)
	{
		_AST(0);
		//outRect = inRect;
		...
		//{
		//case ALIGN_X_Y:
		//	p.redefine(VEC3::UNIT_Z, VEC3(0, 0, vec.z < 0.0 ? minHeight : maxHeight));
		//	break;
		//case ALIGN_X_Z:
		//	p.redefine(VEC3::UNIT_Y, VEC3(0, vec.y < 0.0 ? minHeight : maxHeight, 0));
		//	break;
		//case ALIGN_Y_Z:
		//	p.redefine(VEC3::UNIT_X, VEC3(vec.x < 0.0 ? minHeight : maxHeight, 0, 0));
		//	break;
		//}
		...
		//VEC3 corners[4];
		//float startHeight = verticalVal < 0.0 ? maxHeight : minHeight;
		...
		//	std::pair<bool, float> rayHit = ray.intersects(p);
		//	if (rayHit.first)
		//	{
		//		VEC3 pt = ray.getPoint(rayHit.second);
	...
	PixelBox* Terrain::calculateLightmap(const Rect& rect, const Rect& extraTargetRect, Rect& outFinalRect)
	{
		...
		//const VEC3& lightVec = TerrainGlobalOptions::getSingleton().getLightMapDirection();
		...
		//float heightPad = (getMaxHeight() - getMinHeight()) * 1.0e-3f;
		...
		//		// than world size
		//		std::pair<bool, VEC3> rayHit = rayIntersects(ray, true, mWorldSize);
	...
	//---------------------------------------------------------------------
	void Terrain::updateCompositeMapWithDelay(float delay)
	{
		...
		bool recalculate /*= false*/, bool notifyOther /* = true */)
	{
		...
		{
			_AST(neighbour != this && "Can't set self as own neighbour!");
	...
	{
		_AST(mSize == getNeighbour(index)->getSize());
	...
	//---------------------------------------------------------------------
	void Terrain::getPointFromSelfOrNeighbour(long x, long y, VEC3* outpos) const
	{
	...
	//---------------------------------------------------------------------
	//Terrain* Terrain::raySelectNeighbour(const Ray& ray, float distanceLimit /* = 0 */)
	//{
	...
	//	std::pair<bool, float> intersectResult;
	//	if (tDir.x < 0.0f)
	//	{
	//		intersectResult = Math::intersects(terrainRay, Plane(VEC3::UNIT_X, VEC3::ZERO));
	//		if (intersectResult.first && intersectResult.second < dist)
	//			dist = intersectResult.second;
	//	}
	//	else if (tDir.x > 0.0f)
	//	{
	//		intersectResult = Math::intersects(terrainRay, Plane(VEC3::NEGATIVE_UNIT_X, VEC3(1, 0, 0)));
	//		if (intersectResult.first && intersectResult.second < dist)
	//			dist = intersectResult.second;
	//	}
	//	if (tDir.y < 0.0f)
	//	{
	//		intersectResult = Math::intersects(terrainRay, Plane(VEC3::UNIT_Y, VEC3::ZERO));
	//		if (intersectResult.first && intersectResult.second < dist)
	//			dist = intersectResult.second;
	//	}
	//	else if (tDir.y > 0.0f)
	//	{
	//		intersectResult = Math::intersects(terrainRay, Plane(VEC3::NEGATIVE_UNIT_Y, VEC3(0, 1, 0)));
	//		if (intersectResult.first && intersectResult.second < dist)
	//			dist = intersectResult.second;
	//	}
	...
	//	VEC3 terrainIntersectPos = terrainRay.getPoint(dist);
	...
	void Terrain::Render()
	{
		getPointTransform(&m_cbTerrain.posIndexToObjectSpace);
		m_cbTerrain.posIndexToObjectSpace = m_cbTerrain.posIndexToObjectSpace.Transpose();
		m_cbTerrain.baseUVScale = 1.0f / (getSize() - 1);

		uint32 numLayers = getLayerCount();
		uint32 numUVMul = numLayers / 4;
		if (numLayers % 4)
			++numUVMul;

		m_cbTerrain.uvMul_0 = VEC4(getLayerUVMultiplier(0),getLayerUVMultiplier(1),getLayerUVMultiplier(2),getLayerUVMultiplier(3));

		m_pConstantBuf->UpdateBuf(&m_cbTerrain);
		m_pConstantBuf->Apply(10, true);
	...
	{
		size_t sz = 0;
		if (/*_getUseVertexCompression()*/true)
		{
			// short2 position
			sz += sizeof(short) * 2;
			// float1 height
	...
	//---------------------------------------------------------------------
	void Terrain::_populateIndexBuffer(uint32* pI, uint16 batchSize,
	...
	void Terrain::DefaultGpuBufferAllocator::allocateVertexBuffers(Terrain* forTerrain,
		size_t numVertices, VertexBuffer** destPos, VertexBuffer** destDelta)
	{
	...
	//---------------------------------------------------------------------
	VertexBuffer* Terrain::DefaultGpuBufferAllocator::getVertexBuffer(
		VBufList& list, size_t vertexSize, size_t numVertices)
	{
		...
		// Didn't find one?
		return g_env.pRenderer->GetRenderSys()->CreateVertexBuffer(sz, vertexSize, nullptr, 0);
	}
	//---------------------------------------------------------------------
	void Terrain::DefaultGpuBufferAllocator::freeVertexBuffers(VertexBuffer* posbuf, VertexBuffer* deltabuf)
	{
		mFreePosBufList.push_back(posbuf);
		mFreeDeltaBufList.push_back(deltabuf);
	}
	//---------------------------------------------------------------------
	IndexBuffer* Terrain::DefaultGpuBufferAllocator::getSharedIndexBuffer(uint16 batchSize,
		uint16 vdatasize, size_t vertexIncrement, uint16 xoffset, uint16 yoffset, uint16 numSkirtRowsCols,
		uint16 skirtRowColSkip, uint32& nIndexCount)
	{
